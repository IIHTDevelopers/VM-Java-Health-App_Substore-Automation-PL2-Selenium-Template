...
public class substore_testcase extends AppTestBase {
	...
	@Test(priority = 21, groups = {
			"sanity" }, description = "Pre condition: User should be logged in and it is on Inventory sub-module\r\n"
					...
					+ "3. Enter all the required field \r\n" + "(Select Inventory, Item Name, ReturnQty)\r\n"
					+ "4. Click on \"Return\" button\r\n"
					+ "5. After clicking the \"Return\" button, a success message stating \"Return is Generated and Saved\" should be displayed. Ensure that the entered data is visible in the \"Return\" section of the \"Inventory\" sub-module.")
	public void verifyReturnModule() throws Exception {
		substorePageInstance = new substore_page(driver);
		Map<String, String> substoreExpectedData = new FileOperations().readJson(expectedDataFilePath, "substore");

		Assert.assertEquals(substorePageInstance.verifyReturnModule(substoreExpectedData.get("returnMsg")),
				substoreExpectedData.get("returnMsg"));
